...
class AdminListBuilder extends AdminBuilder
{
    ...
    */
    ...
    public function keyFile($name, $title)
    {
    	return $this->key($name, $title, 'file');
    }
    ...
    public function display($solist = '')
    {
        ...
        $this->convertKey('file', 'html', function ($value, $key, $item) {
        	
        	$html = '';
        	
        	if (!empty($value)) {
        		$id_array = str2arr($value);
        	} else {
        		return "";
        	}
        	...
        	foreach ($id_array as $file_id) {
	            if(intval($file_id)){//value是文件id
	                ...
	                if ($file_info) {
	                	...
	                	$file_download = U('/Admin/File/downloadFile', array('file_id'=>$file_id));
	                	 
	                	$html="<div><a href=\"$file_download\" target=\"_blank\">$file_name</a> &nbsp; &nbsp;(md5: $file_md5) </div>" . $html;
	                }	                
	            }
        	}
            return $html;
