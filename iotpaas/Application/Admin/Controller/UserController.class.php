...
class UserController extends AdminController
{
    ...
    public function addNewAdmin() {
    	if (IS_POST) {
    		...
    		$userApi = new UserApi();
    		$result = $userApi->register($username, $nickname, $password, $email);
    	    if ($result>0) {
    	    	$this->success($result, U('index'));
    	    } else {
    	    	$ucentermodel = D('User/UcenterMember');
    	    	$this->error( $ucentermodel->getErrorMessage($result));
    	    }
    	    
    	} else {
    		$builder = new AdminConfigBuilder();
    		$builder->title('新建管理台用户');
    		$builder->keyText('username', '用户名', '系统登陆名')->keyText('nickname','昵称');
    		$builder->keyText('password', '密码')->keyText('repassword', '再次输入密码');
    		$builder->keyText('email', '邮箱');
    		
    		$builder->buttonSubmit(U('addNewAdmin'));    		
    		$builder->display();
    	}
    }
