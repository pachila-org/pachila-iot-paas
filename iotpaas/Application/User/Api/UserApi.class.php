...
class UserApi extends Api{
    ...
    public function login($username, $password, $type = 1){
        if(file_exists('./api/uc_login.lock')){
        	\Think\Log::write('Login Path under uc_login.lock');
        	...
        	\Think\Log::write('Login Path with no uc_login.lock');
            if(UC_SYNC && $username != get_username(1)){
            	\Think\Log::write('Login Path with UC_SYNC'.UC_SYNC);
                return $this->ucLogin($username, $password);
            }
            \Think\Log::write('Login Path without UC_SYNC'.UC_SYNC);
    ...
    {
    	//检查旧密码是否正确
    	if (!$this->model->verifyUser($uid, $old_password)) {
    		$this->error = -41;
    		return false;
    	}
    	//更新用户信息
    	...
    	$this->model->where(array('id' => $uid))->save($data);
    	//返回成功信息
    ...
    public function getErrorMsg($error_code) {
    	return $this->model->getErrorMessage($error_code);
    }
